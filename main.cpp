...
int main(int argc, const char* argv[])
{
    ...
    const std::unique_ptr<fi_info, decltype(&fi_freeinfo)> hints_p(fi_allocinfo(), fi_freeinfo);
    hints_p->ep_attr->type = FI_EP_MSG;
    std::cout << "Hints:\n" << fi_tostr(hints_p.get(), FI_TYPE_INFO);
    fi_info* info{nullptr};
    std::cout << info << "\n";
    fi_getinfo(fi_version(), hostname, nullptr, 0 /*FI_PROV_ATTR_ONLY*/, hints_p.get(), &info);
    std::cout << info << "\nAccess domains:\n";
