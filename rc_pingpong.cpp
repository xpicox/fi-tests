...
static void usage(char *argv0) {
  ...
  FT_PRINT_OPTS_USAGE(
      "-r <rx-depth>",
      "number of receives to post at a time (default 500)");
...
int main(int argc, char *argv[]) {
  ...
  if (rx_depth) {
    ...
    rx_depth =
        (rx_depth_default > fi->rx_attr->size) ?
  ...
  while (rcnt < iters || scnt < iters) {
    ...
    switch ((int) (uintptr_t) wc.op_context) {
      ...
      default:
        fprintf(
            stderr,
            "Completion for unknown wc_id %d\n",
  ...
  err3: fi_shutdown(ctx->ep, 0);
  err2: ret = pp_close_ctx(ctx);
  if (!rc)
    rc = ret;
  err1: fi_freeinfo(fi);
  return rc;
